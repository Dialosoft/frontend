...
export async function PostCategory({ name, description, categoryId, type }:CategoryProps) {
	...
	try{
		...
		const response = await axios.post("http://gateway-service:8080/dialosoft-api/management-service/create-category",{ categoryId:categoryId,description: description,
			name: name,
			type:type,
